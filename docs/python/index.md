...
The Python port is available on PyPI:
...
The Python port uses the same word lists and matching rules as the JavaScript package, so a piece of text gets the same result in every language. That covers detecting and censoring, the language and strictness options, and the checks that keep real names from being flagged.
